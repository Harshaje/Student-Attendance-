...
function createAttendanceSession(){
  ...
  const subject=db.subjects.find(s=>s.id===subjectId);if(!subject)return;
  const session={id:makeId("SES"),subjectId,title,date:today(),startTime:nowTime(),endTime:null};
  ...
  document.getElementById("sessionQR").innerHTML="<p class='ok'>Subject selected. Scan each student's personal QR code below.</p>";
  document.getElementById("scanMsg").textContent=`Attendance ready for ${subject.name}. Students use their single personal QR.`;
}
...
function downloadSessionQR(){alert("A subject QR is not required. Each student has one permanent personal QR.");}
function shareSessionQR(){alert("A subject QR is not required. Each student has one permanent personal QR.");}

async function startScanner(){
  if(scanner)return;if(!activeSession)return alert("Please select a subject and start attendance first.");
...
function markAttendance(qr){
  ...
  if(db.attendance.some(a=>a.studentId===student.id&&a.subjectId===activeSession.subjectId&&a.date===activeSession.date)){document.getElementById("scanMsg").textContent=`Already marked for ${student.name} in this subject today.`;return}
  const now=new Date();
...
function renderSummary(rows){
  ...
  summary.innerHTML=`<p><b>Subject:</b> ${esc(subjectId?((db.subjects.find(s=>s.id===subjectId)||{}).name||""):"All Subjects")} &nbsp; <b>Period:</b> ${esc(reportPeriodLabel())}</p><div class="summaryCards"><div class="summaryCard"><b>Total Classes</b><strong>${sessions.length}</strong></div><div class="summaryCard"><b>Total Students</b><strong>${students.length}</strong></div><div class="summaryCard"><b>Attendance Records</b><strong>${rows.length}</strong></div></div><div class="tableWrap"><table><thead><tr><th>Roll</th><th>Student</th><th>Present</th><th>Total Classes</th><th>Attendance %</th></tr></thead><tbody>${data.map(x=>`<tr><td>${esc(x.student.rollNo)}</td><td>${esc(x.student.name)}</td><td>${x.present}</td><td>${x.total}</td><td><strong>${x.pct}%</strong></td></tr>`).join("")}</tbody></table></div>`
}
